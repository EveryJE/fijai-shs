            ...
            <Table>
                <TableHeader>
                    <TableRow className="bg-primary-50/50 hover:bg-primary-50/50">
                        <TableHead className="w-[250px] pl-6 font-bold uppercase text-[11px] tracking-widest">Full Name</TableHead>
                        <TableHead className="font-bold uppercase text-[11px] tracking-widest">Email Identifier</TableHead>
                        <TableHead className="font-bold uppercase text-[11px] tracking-widest">Institutional Roles</TableHead>
                        <TableHead className="font-bold uppercase text-[11px] tracking-widest">Onboarded</TableHead>
                        <TableHead className="text-right pr-6 font-bold uppercase text-[11px] tracking-widest">Actions</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {filteredProfiles.length === 0 ? (
                        ...
                        filteredProfiles.map((profile: any) => (
                            <TableRow key={profile.id} className={cn(
                                ...
                                <TableCell className="pl-6 py-4">
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-2">
                                            <span className="font-bold text-sm tracking-tight">{profile.fullName || "Anonymous"}</span>
                                            {!profile.isActive && (
                                                <span className="text-[9px] font-black uppercase tracking-tighter bg-red-100 text-red-700 px-1.5 py-0.5 rounded">Inactive</span>
                                            )}
                                        </div>
                                        <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium mt-0.5">{profile.classYear ? `Class of ${profile.classYear}` : "Institutional Member"}</span>
                                    </div>
                                </TableCell>
                                <TableCell className="text-sm font-medium text-muted-foreground/80 lowercase">{profile.email}</TableCell>
                                <TableCell>
                                ...
                                <TableCell className="text-muted-foreground text-[11px] font-bold uppercase tracking-wider">
